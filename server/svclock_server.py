...
for lock in config["locks"]:
    ...
    if lock["hello_time"] >= lock["dead_time"]:
        raise ValueError(
            f"Lock {lock['name']} hello timer greater or equal to dead time"
        )
...
def lock_api():
    ...
    for name, status in lock_manager.items():
        ...
        if name.encode("utf-8").hex() == lock_name.encode("utf-8").hex():
            ...
            if (
                ...
                > status[1] + datetime.timedelta(seconds=l["dead_time"])
            ):
    ...
    r = {"active": active, "hello": hello_time, "dead": dead_time}
    return jsonify(r)
